...
class Interpreter:
    ...
    def _load_model(self, model: ast.ModelDecl) -> None:
        for param in model.params:
            init_fn = self._resolve_init(param.init)
    ...
    def _bind_args(
        ...
        if len(args) != len(fn.params):
            raise InterpreterError(
                f"Expected {len(fn.params)} arguments, got {len(args)}"
            )
    ...
    def _exec_stmt(self, stmt: ast.Stmt, ctx: ExecContext) -> None:
        ...
        if isinstance(stmt, ast.LossStmt):
            if ctx.last_graph is None:
                raise InterpreterError("loss requires a preceding graph")
            if not _loss_expr_is_graph_only(stmt.expr, ctx.last_graph.name_to_id):
                raise InterpreterError("loss expression must reference graph-computed names only")
            executor = GraphExecutor(
                self.rules, self.params, source=self.program.source, filename=self.program.filename
            )
        ...
        if isinstance(stmt, ast.GradStmt):
            if ctx.last_graph is None:
                raise InterpreterError("grad requires a preceding graph")
        ...
        if isinstance(stmt, ast.StepStmt):
            ...
            if not isinstance(grad, Grad):
                raise InterpreterError(f"Unknown grad '{stmt.grad_name}'")
        ...
        if isinstance(stmt, ast.ExplainStmt):
            ...
            if stmt.output_path:
                ...
                if not isinstance(out_value, str):
                    raise InterpreterError("Explain output path must be a string")
                self._render_explain_output(ctx.last_explain, out_value)
            return
        if isinstance(stmt, ast.FetchStmt):
            if ctx.last_graph is None:
                raise InterpreterError("fetch requires a preceding graph")
            if self._is_param_name(stmt.target_name):
                raise InterpreterError(f"Cannot assign to param '{stmt.target_name}'")
            try:
        ...
        if isinstance(stmt, ast.AssignStmt):
            if self._is_param_name(stmt.name):
                raise InterpreterError(f"Cannot assign to param '{stmt.name}'")
    ...
    def _eval_optimizer(self, call: ast.CallExpr, ctx: ExecContext) -> Optimizer:
        if call.func != "SGD":
            raise InterpreterError(f"Unknown optimizer '{call.func}'")
        lr = None
        for arg in call.args:
            ...
            if arg.name == "lr":
                ...
                lr = float(value)
        if lr is None:
            raise InterpreterError("SGD requires lr")
        return SGD(lr=lr)

    def _resolve_init(self, init_expr: Optional[ast.Expr]):
        ...
        raise InterpreterError("Unsupported init expression")
    ...
    def _eval_const_expr(self, expr: ast.Expr) -> float:
        ...
        raise InterpreterError("Initializer args must be numeric literals")
    ...
    def _eval_host_expr(self, expr: ast.Expr, ctx: ExecContext) -> object:
        ...
        if isinstance(expr, ast.IndexExpr):
            ...
            try:
                ...
                raise InterpreterError(f"Indexing failed: {exc}") from exc
        if isinstance(expr, ast.Name):
            ...
            try:
                ...
                pass
            raise InterpreterError(f"Unknown name '{expr.value}'")
        if isinstance(expr, ast.UnaryOp) and expr.op == "-":
